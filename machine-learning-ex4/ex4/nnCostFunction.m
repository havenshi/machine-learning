...
% construct Y, Y is [1 0 ... 0]', [0 1 ... 0]' ... m columns, means class of each m * y
I = eye(num_labels);
Y = zeros(m, num_labels); % 5000rows * 10classes
...
% calculate J(theta), ',2' means sum each rows
J = sum(sum((-Y).*log(h) - (1-Y).*log(1-h), 2))/m + lambda*penalty/(2*m);


% backpropagation, calculate sigma
sigma3 = a3 - Y;
...
Theta1_grad = delta1./m + penalty1;
Theta2_grad = delta2./m + penalty2;
